...
export function addFiles(files: FileList | File[], target?: Key) {
...
export async function loadExample(ex: Example) {
  ...
  try {
    ...
    const comps = state.competitors.length
      ? []
      : await Promise.all(ex.competitors.map((c, i) => exampleFile(c.url, `Конкурент ${i + 1}.jpg`)))
