...
WEIGHTS = {"focus": 0.35, "ease": 0.25, "thumb": 0.25, "contrast": 0.15}
...
# Лёгкость восприятия: 3 элемента -> считывается сразу, 16 -> перегруз; 7 заметных цветов -> спокойно, 22 -> пёстро.
EASE_ELEMENTS = (16, 3)
EASE_COLORS = (22, 7)
...
NOTE_FOCUS = (40, 70)
NOTE_EASE = (25, 45)  # ниже первого — «плохо», ниже второго — «внимание»
NOTE_THUMB = (45, 75)
NOTE_CONTRAST = 35
...
def complexity(rgb: np.ndarray) -> dict:
    ...
    ease = 0.7 * scale(elements, *EASE_ELEMENTS) + 0.3 * scale(colors, *EASE_COLORS)
    return {"elements": elements, "colors": colors, "ease": round(ease)}
...
def notes(s: dict, r: dict) -> list[dict]:
    ...
    if s["ease"] < NOTE_EASE[1]:
        add(
            "bad" if s["ease"] < NOTE_EASE[0] else "warn",
            "Визуальный перегруз",
