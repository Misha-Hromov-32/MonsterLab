...
def test_showcase_is_built_for_the_first_example(client: TestClient) -> None:
    body = client.get("/api/public/showcase").json()
    assert body["card"]["index"] >= 0
...
def test_showcase_failure_is_not_retried_on_every_visit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(showcase, "_build", boom)
    ex = {"id": "broken", "hero": None, "variants": [{"id": "v"}], "competitors": []}
    assert asyncio.run(showcase.get(ex)) is None
    assert asyncio.run(showcase.get(ex)) is None
    assert calls == ["broken"]
