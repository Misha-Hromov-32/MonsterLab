...
│   ├── accounts.py     покупатели: вход по email, тариф, дневные лимиты платных функций
│   ├── billing.py      оплата подписки через ЮKassa, проверка уведомлений
│   ├── improve.py      улучшенная обложка: генерация по выводам разбора
│   ├── marketplace.py  конкуренты из выдачи Wildberries (headless Chromium)
│   ├── db.py           app.sqlite: покупатели, платежи, расход лимитов, кэш выдачи
│   ├── expert.py       экспертный разбор через ProxyAPI, рейтинг Брэдли–Терри
│   ├── showcase.py     витрина главной: обложка и лента первого примера
│   └── precompute.py   заранее посчитанные результаты примеров в SQLite
...
![Порядок взгляда на обложке](images/analyze-gaze.jpg)
...
| **Лёгкость восприятия** | 0,7 · элементы + 0,3 · цвета: число отдельных элементов (связные области контуров) и заметных цветов (ячейки гистограммы Lab 8×8×8, занимающие больше 0,8% кадра) | 16 элементов → 3, 22 цвета → 7 |
...
**Индекс = 0,35 · фокус + 0,25 · лёгкость восприятия + 0,25 · превью + 0,15 · контраст.** Это эвристика для
...
![Тест полки среди конкурентов](images/shelf.jpg)
...
├── examples.sqlite        заранее посчитанные результаты примеров (пересоздаётся сама)
├── app.sqlite             покупатели, платежи, расход дневных лимитов, кэш выдачи Wildberries
├── competitors/           картинки найденных конкурентов (кэш на сутки)
...
## Примеры считаются заранее

Показ примера не должен тратить процессор: на слабом сервере нейросеть считает тест полки
около минуты. Поэтому для каждого опубликованного примера всё — разбор каждой обложки,
тест полки для обеих раскладок и витрина главной — считается один раз в фоновом потоке
(`services/precompute.py`) и хранится в `examples.sqlite`.

Пересчёт запускается сам: у каждой записи есть отпечаток примера — состав и порядок картинок,
подписи, обложка для главной, движок внимания и версия алгоритмов (`ALGO_VERSION`). После любой
правки в админке, новой ревизии встроенных примеров или изменения метрик отпечаток меняется,
и пример считается заново. Пока расчёт не готов, `GET /api/public/examples/{id}/results`
отвечает `{"ready": false}`, и интерфейс считает пример как обычную загрузку.

Готовые результаты приходят вместе с id загрузок: обложки примера кладутся в очередь загрузок,
поэтому экспертный разбор и полка со своими конкурентами работают без повторного анализа.

## Платные функции и подписка

Анализ обложек, тест полки и примеры бесплатны и без входа. Три функции тратят деньги или ресурсы
сервера, поэтому доступны после входа и ограничены дневным лимитом тарифа (настраивается в админке):

| Функция | Как работает |
|---|---|
| **Экспертный разбор** | мультимодальные модели через ProxyAPI; в сравнении — **вероятность выбора**: сила варианта по модели Брэдли–Терри, то есть шанс, что покупатель выберет его, увидев все варианты рядом |
| **Улучшенная обложка** | исходник и выводы разбора (плюс замечания экспертов, если они есть) уходят в `/images/edits` модели генерации (`IMAGE_MODEL`); товар, бренд и смысл текстов модель сохраняет |
| **Подбор конкурентов** | выдача Wildberries по запросу открывается в headless Chromium (у WB антибот-защита, простые HTTP-запросы блокируются); картинки качаются с открытого CDN WB. Ozon блокирует и headless-браузер — пока только вручную |

Лимит списывается только после успешного запуска: сбой модели или маркетплейса лимит не тратит.

Оплата — через ЮKassa. Сервис создаёт платёж и отправляет покупателя на страницу оплаты. Уведомлению
ЮKassa он не верит: заново запрашивает платёж по id и, если тот действительно оплачен, один раз
продлевает подписку. Повторные уведомления подписку повторно не продлевают.
